import React from 'react';
import { Button, TextField, Grid, Typography } from '@material-ui/core';
import { inject, observer } from 'mobx-react';

import './Homepage.css';

@inject('store') @observer
class Homepage extends React.Component {
  constructor(props) {
    super(props);
    this.fooStore = this.props.store.fooStore;
  }

  render() {
    return (
      <Grid container direction="column" class="container">
        <Grid 
          item 
          container
          direction="row"
          alignItems="center"
          spacing={24}
        >
          <Grid item>
            <TextField 
              placeholder="Foo"
              id="foo"
              onChange={this.fooStore.setValue}
            ></TextField>
          </Grid>
          <Grid item>
            <Button 
              onClick={this.fooStore.save} 
              variant="outlined"
            >
                Foo
            </Button>
          </Grid>
        </Grid>
        <Grid 
          item
          container
          direction="column"
          id="formContainer"
        >
          <Grid item>
            <Typography variant="title">Store value: {this.fooStore.values.foo}</Typography>
          </Grid>
          <Grid item>
            <Typography variant="title">Database id: {this.fooStore.values.id}</Typography>
          </Grid>
        </Grid>
        <Grid item>
          <img className="logo" src="data:image/png;base64," alt="Feathers Logo" />
          <h2 className="center-text">A REST and realtime API layer for modern applications.</h2>
  
          <footer>
            <p className="center-text">For more information on Feathers see <a href="https://docs.feathersjs.com" title="Feathers Documentation" target="blank">docs.feathersjs.com</a>.</p>
          </footer>
        </Grid>
      </Grid>
    );
  }
}

export default Homepage;